...
def test_regions_fraction(regions,bbox,cutoff = 0.5):
    ...
    for region in regions:
        ...
        overlap = intersection  / (area)#+region_area - intersection + 1e-07)
...
def evaluate_mot(preds,gts,ignored_regions = [],threshold = 100,ignore_threshold = 0.5):
    ...
    for frame in range(len(gts)):
        ...
        for i,obj in enumerate(pred):
            ...
            try:
                exclude = test_regions_fraction(ignored_regions[frame],obj['bbox'], cutoff = ignore_threshold)
            except:
                pass
            
            if not exclude:
                if obj["class_num"].item() in [1,7]:
        ...
        if False: # use distance for matching
            ...
            targets = [gt[i]["bbox"] for i in range(len(gt_ids))]
            if len(targets) == 0:
                targets = [torch.zeros([4])]    
            targets = torch.stack(targets).double()
            
            outputs = [torch.from_numpy(pred[pred_idxs[j]]["bbox"]) for j in range(len(pred_ids))]
            if len(outputs) == 0:
                outputs = [torch.tensor([0,0,0,0])]
            outputs = torch.stack(outputs).double()
            
            # should each be n_gts x n_preds tensor
            minx = torch.max(targets[:,0].unsqueeze(1).repeat(1,len(pred_ids)), outputs[:,0].unsqueeze(0).repeat(len(gt_ids),1))
            miny = torch.max(targets[:,1].unsqueeze(1).repeat(1,len(pred_ids)), outputs[:,1].unsqueeze(0).repeat(len(gt_ids),1))
            maxx = torch.min(targets[:,2].unsqueeze(1).repeat(1,len(pred_ids)), outputs[:,2].unsqueeze(0).repeat(len(gt_ids),1))
            maxy = torch.min(targets[:,3].unsqueeze(1).repeat(1,len(pred_ids)), outputs[:,3].unsqueeze(0).repeat(len(gt_ids),1))
            
            zeros = torch.zeros(maxy.shape).double()
            intersection = torch.mul(torch.max(zeros,maxx-minx),torch.max(zeros,maxy-miny))
            areaA = torch.mul(targets[:,2]-targets[:,0],targets[:,3]-targets[:,1]).unsqueeze(1).repeat(1,len(pred_ids))
            areaB = torch.mul(outputs[:,2]-outputs[:,0],outputs[:,3]-outputs[:,1]).unsqueeze(0).repeat(len(gt_ids),1)
            union = areaA + areaB - intersection
            iou = torch.div(intersection,union)
            dist = (1-iou).data.numpy()
